...
from utils.database import mongo_client, redis_client
from aws.s3_client import get_boto3_client
...
def get_prompt_from_mongo(**kwargs:dict):
    ...
    mongo = mongo_client(collection=collection)
...
def save_id(session_key, message, insert_mongo, indepth):
    ...
    if pid and type(pid) == str:
        ...
        mongo_client(collection=os.getenv('DB_PROMPT_COLL_NAME')).update_one(query=query, update=update)
...
def save_s3(s3_save:dict, data:str):
    ...
    baAIReport = mongo_client(collection='baAIReport')
